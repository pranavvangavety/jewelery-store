...
public class NotificationService {
    ...
    public void sendEmailVerification(EmailVerificationEvent event) {
        ...
        String body = "<h2>Welcome to Trinket Story, " + esc(event.getFirstName()) + "!</h2>"
                + "<p> Please verify your email by clicking the link below:</p>"
                + "<a href='http://localhost:5173/verify-email?token=" + esc(event.getToken())  + "'> Verify Email</a>"
    ...
    public void sendPasswordReset(PasswordResetEvent event) {
        ...
        String body = "<h2>Hi " + esc(event.getFirstName()) + ",</h2>"
                + "<p>Click the link below to reset your password: </p>"
                + "<a href = 'http://localhost:5173/reset-password?token=" + esc(event.getToken()) + "'>Reset Password</a>"
